...
export default function Comment({comment, onCommentApproved}){
    ...
    return (
       <div className="max-w-md mx-auto my-4 bg-white border rounded-lg shadow-md">
            <div className="p-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">{comment.post.title }</h3>
                
                <div className="text-sm text-gray-500 flex justify-between items-left flex-col mb-4">
                <span className="block">By: <span className="font-medium">{comment.author.name }</span></span>
                <span className="block">{commentCreatedDate.toLocaleString()}</span>
                </div>
